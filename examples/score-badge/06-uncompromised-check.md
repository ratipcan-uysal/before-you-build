...
1. **`03-answers.md` said the re-score moves the document to 65.** Recomputed from the item scores in `02`: P2 2→3 gives K1 16/21×20, B4 2→3 gives K3 16/24×25, total **64**. *(Corrected.)*
2. **`02-readiness-score.md` said three of the five `capability` items scored 3.** Its own arithmetic block lists four — Y1, Y2, Y3 and Y5. *(Corrected.)*
...
8. **"0 questions lost."** Two were. `01-idea-grill.md` names *"is the QA tool in the first wave?"* and marks it **"named here so it is not mistaken for settled"**; `00` raises it too. `05` carries a different QA-tool question and says one decision remains undecided.
...
14. **The "3 of 4" rounding count** does not survive `03`'s answer that the seller panel already rounds correctly and one other surface also rounds to one decimal. The weakest of the fourteen, and the reviewer said so.
...
**`CONDITIONAL` still stands.** `02` scored a document that came from outside the session, its arithmetic reconciles, and the two errors the reviewer found in it are prose miscounts that move no score. That verdict was never compromised and is not withdrawn.
...
Twelve of the fourteen were invisible from inside and obvious from outside. The reviewer had less context, less investment, and no memory of why any decision was made — and that was the entire advantage.
