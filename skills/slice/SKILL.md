...
**A fifth outcome, for the decision that is not yours to take.** *Cut the build, never the decision* assumes you are allowed to take the decision. Where it belongs to law, to money, or to a named function, you are not. Those rows go to their owner as `[DECISION NEEDED]`, in their own list — not into *decided now, built later*, which reads as settled and invites the next reader to settle it. On a measured run eighteen legal calls landed in that section, and the way a careless reader clears them is by making them.

**Every row of Phase 0's list ends in one of these five**, and the last thing you do is check that. A row that appears on the opening table and nowhere else is not a decision, it is an oversight with a table around it — and downstream nothing will ever quote it out of scope, so it sits in the score at zero forever.
