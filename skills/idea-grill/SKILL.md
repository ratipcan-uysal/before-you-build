...
4. **Fire once. Stop. Wait** — *for an answer, from whoever can give one.* **With nobody there, the document is the defendant:** put the provocation, answer it from the material and only the material, and rule on that answer the way you would rule on a person's. What you must not do is supply the defence a person would have made and then beat it — a grill that writes both sides is theatre, and it always convicts. Where the material cannot answer, the outcome is `unanswered` and the question goes to the owner. That is the result, not a failure to finish.
...
- **Output:** run the whole session in chat. At the close, offer to save the synthesis to a file. Never write files unprompted — **unless you were called with somewhere to put it.** A chain step handed an output path writes there and says so.
