...
**If the caller is unclear, ask which of the three you are doing** before reading anything. They need different sources and produce different documents, and doing all three at once produces a survey nobody acts on. **With nobody to ask, the material decides it:** a specified mechanism nobody examined is the first, a constraint with no owner is the second, an unchecked claim about a platform or vendor is the third. Where two are present, take the one the work rests on more heavily, name the choice in your first line, and say what a run of the other would have looked for.
