...
**The document order is not the working order, and only one of them is written down here.** The split below is the first thing a reader sees and the last thing you know: work the phases, then open with the count. Writing the number before the sweep produces findings fitted to a total, which is the failure this skill is built against.
