...
**Load in one pass, before Phase 0:** `references/entities.md`. **References together; documents one at a time.** Opening five references separately costs a round trip each and every round trip re-sends everything read so far. Chain documents are the opposite case: they run past four hundred lines, a batched read of several of them overflows a single read and comes back as more turns than it saved, and each one opens with a carrier that tells you which part you need. Read the carrier, then the part. Left for later: `references/diagram.md` only if a diagram is drawn.
...
**A hand-off with its own error path is not a hand-off any more.** The rule below is right that passing work to another system writes nothing here — until the flow draws what happens when the pass fails. A failure somebody can be shown, retry, or count has a state, and a state is held somewhere. Where the flow gives a hand-off an error path and no entity, that is a finding, not a clean sweep.
...
At four or more entities, ask once whether they want one. **With nobody to ask, draw it only if something downstream reads a diagram** — a pack being assembled, a handoff to people rather than to a generator. The entity count says a diagram would be legible; it does not say anyone wants one, and an unread diagram costs its own reference file to produce. Conventions, and the same check applied to the drawing: [`references/diagram.md`](references/diagram.md).
...
**Open with the carrier.** The entities, the identity rule for each, where the stored-versus-computed table is, and the open decisions with owners. `api-needs` reads the nouns and the identity rules; it does not need the five questions that produced them.
...
- **An upstream document may name your subject; it does not choose it.** A carrier that writes *"this goes to you, start with these"* is telling you where the pressure looked greatest from where it stood. Read it, then test it against the material — on a measured run that instruction, followed literally, would have returned a quarter of the work and hidden the run's largest finding. Say in one line which you took.
...
- **An entity nobody can name is not an entity.** If the material calls it three things, that is a finding — put the three names in front of the user and make them choose one. With nobody to choose, carry all three, say they are one thing, and make it a `[DECISION NEEDED]` with an owner.
