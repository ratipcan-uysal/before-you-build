...
- **An upstream document may name your subject; it does not choose it.** A score or a radar that writes *"this table goes to `decision-memo`"* is telling you where it thinks the pressure is, from where it stood. Read it, then test it against what is actually blocking now — on a measured run the assignment pointed at five decisions a later cut had already retired, while the real bottleneck sat in a document that never addressed you. Say which you took and why, in one line.
