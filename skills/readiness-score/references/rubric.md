...
- **The same rule covers a lean the author disowned.** *"Probably first-come, but we have not decided that"* carries exactly what `[ASSUMED] first-come` carries, and one word more: that nobody has agreed to it. Score them the same. Scored apart, the honest sentence loses a point to the marked guess it already contains, and the author learns to stop writing the second half.
- **And disclosing a dependency must not widen the classification.** A request that names the integration it needs, and the person who owns it, has told you about a dependency — not committed to a second surface. Classify on the surfaces the document commits to building. A named dependency is evidence under K4, where it scores. Read the other way, the request that mentions nobody scores higher than the one that names an owner, and on a measured run that gap was six points.
- **What these three have in common is worth stating once.** Every one of them charges a page for being candid, and each is invisible in a single score. When a rule you are about to apply would score the vaguer document higher than the franker one, that is the finding — write it in *Arithmetic* rather than absorbing it.
