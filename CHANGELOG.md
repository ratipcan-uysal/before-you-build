...
**Heads up — the rubric was charging honest documents and it says so in its own file.** `readiness-score`'s rubric already names the trap for one case: a rule the document contradicts *"charges an honest author who quoted the contradiction and refused to resolve it, because resolving it with an invented `[ASSUMED]` would have scored higher."* Two independent scorings of the same request found the same penalty running in three more places, and none of them was closed.

- *"Probably first-come, but we have not decided that"* scored 0 while `[ASSUMED] first-come` scores 1. Identical information, one word more of it in the honest version, one point less.
- `scoring.md`'s worked example scored *"handle errors gracefully"* as 1 — mentioned, not specified — on the item that fires the blocker. A document admitting it had not worked out its error paths scored 0 and was blocked. **Four words that say nothing bought a pass the honest sentence could not.** That example now scores 0, the blocker fires, and the fragment says why.
- Naming the integration and the person who owns it made `multi-surface` the natural reading, opened eight items, seven scored zero, and cost six points. Disclosing a dependency is evidence under K4, not a second surface.

**Three files told readers to do the thing they forbid, and the example is always the half that gets copied.** `flow-map` declares mixing marks and marked steps *"the most common error in this document"* and then prints one combined figure in the coverage line a later skill copies — `api-needs` builds its work list from that line. `request-shaper`'s Phase 1 described autonomous mode as marking every inferred line `[ASSUMED]` while Phase 2 defaults to leaving items open, thirty readiness points apart, and Phase 1 is what a sequential reader hits first. Both are the older description surviving the newer rule.

**The chain had no authority order, and three skills hit it independently.** `request-shaper` had two upstream documents arguing with each other and nowhere to put it; `design-brief` had `slice`'s quotable exclusion against `record.md`'s instruction to draft; `flow-grill` had scope, decisions-within-scope, and an unread original request. `build-context` is told to name every disagreement, and it runs last — the disagreement is born four steps earlier. Ten skills now carry the order, and the instruction to write both statements where it is genuinely unclear.

**Something that exists and is closed to you is a third case, and the set only had two.** v4.6.0 branched two absolute rules on whether the product exists. An unattended run is usually neither: `design-brief` had a live app with 94% adoption and no tokens, no screenshots, nobody to ask — the prohibition applied and its remedy did not. `api-needs` had a grill sitting beside it that nobody handed over. `data-model` had a hand-off with its own error path and a rule saying hand-offs write nothing, so a real gap was recorded as a clean sweep. All three now have the branch.

**Also settled from the same run.** `slice` gained a fifth outcome for the decision it is not authorised to take, after eighteen legal calls landed in a section named *decided now, built later*. `risk-interrogate` gained a lens for work whose rules come from law — the previous regulatory lens was entirely about exposure, and said nothing about a system computing a legal outcome and stamping it — and its strike test now says *listed* means the same breakage, not the same topic. `idea-grill` gained an unattended mode, a merit boundary that keeps the mechanism the value rests on, and the instruction to divide a claimant's numbers by their own base. `flow-grill`'s drift now inherits the severity of what it causes. `ux-grill` gained a scale rule and a place to file a finding that belongs to the record rather than the drawing. `decision-memo` may now derive the date its ask needs, and says that it did.

**And the check written to catch unanswerable rules missed one, in the first new phrasing it met.** `prior-art`'s *"ask which of the three you are doing"* matched none of the sentences in `NEEDS_A_PERSON`, so CI passed a rule that stops an unattended run dead. The same loop had never read `references/`, where most of this set's procedure lives. Both are fixed, and both fixes were verified by putting the fault back and watching them fail. A general pattern was tried first — a person-word near an asking verb — and gave seven false positives against one real hit, because interrogating material in the second person is this set's ordinary voice. That attempt is recorded in the script so nobody rebuilds it.
